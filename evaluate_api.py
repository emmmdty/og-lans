                ...
                if cf_enabled:
                    for _ in range(cf_num_perturb):
                        ...
                        if prompt_style == "simple":
                            ...
                            if args.use_fewshot:
                                cf_messages = ChinesePromptBuilder.get_messages_with_oneshot(
                                    perturbed_text,
                                    num_examples=fewshot_num_examples,
                                    schema=prompt_schema,
                                )
                            else:
                                cf_messages = ChinesePromptBuilder.get_messages_for_inference(
                                    perturbed_text,
                                    schema=prompt_schema,
                                )
                        ...
                        cf_parsed, _ = parse_with_diagnostics(cf_text)
                        cf_events = normalize_pred_events(cf_parsed)
