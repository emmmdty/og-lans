...
def test_runtime_defaults_are_applied(tmp_path):
    ...
    assert loaded["algorithms"]["lans"]["runtime_mode"] == "online_iterable"
    assert loaded["algorithms"]["lans"]["signal_center"] == 0.5
    assert loaded["algorithms"]["lans"]["signal_temperature"] == 0.25
    ...
    assert loaded["training"]["rpo"]["require_valid_labels"] is True
    assert loaded["training"]["preference"]["mode"] == "ipo"
    assert loaded["training"]["preference"]["offset_source"] == "margin_bucket"
    assert loaded["training"]["preference"]["offset_static"] == 0.15
