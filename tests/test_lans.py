...
class TestLANSScheduler:
    ...
    def test_signal_mapping_reaches_high_competence_zone(self):
        """测试新 signal 映射在低损失下可进入高能力区。"""
        scheduler = LANSScheduler(
            ...
            use_ema=False,
            signal_center=0.5,
            signal_temperature=0.25,
            competence_floor=0.05,
            competence_ceiling=0.95,
        )

        high_comp = scheduler.update_competence(0.0)
        low_comp = scheduler.update_competence(1.0)

        assert high_comp > 0.8
        assert low_comp < 0.2
    ...
    def test_get_statistics(self, scheduler: LANSScheduler):
        ...
        assert "signal_center" in stats
        assert "signal_temperature" in stats
