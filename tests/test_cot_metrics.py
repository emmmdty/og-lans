...
def test_counterfactual_consistency_updates_when_old_value_is_replaced():
    evaluator = AcademicEventEvaluator()
    perturbation = {
        "changed": True,
        "old_value": "100亿元",
    ...
    perturbed_pred_events = [
        {
            "event_type": "股份回购",
            "arguments": [
                {"role": "交易金额", "argument": "9999元"},
            ],
        }
    ]
    evaluator.update_counterfactual_consistency(perturbed_pred_events, perturbation)
    report = evaluator.compute_metrics()
    assert report.cot_counterfactual_checked == 1
    assert report.cot_counterfactual_pass_rate == 1.0


def test_counterfactual_consistency_fails_when_old_value_still_present():
    evaluator = AcademicEventEvaluator()
    perturbation = {
        "changed": True,
        "old_value": "100亿元",
    ...
    report = evaluator.compute_metrics()
    assert report.cot_counterfactual_checked == 1
    assert report.cot_counterfactual_pass_rate == 0.0
