...
当前仓库还额外保留了一条面向 DuEE-Fin 旧式文献表格的 legacy comparable track，即 `legacy_dueefin_overall_precision/recall/f1`。这条轨道用于与较早的 DuEE-Fin overall F1 报告方式对齐，但不会替代更严格的 record-aware 主表指标。换句话说，legacy 轨道负责“与旧表可比”，record-aware 轨道负责“反映真实记录组装能力”。

在训练侧，仓库也已经为后续混合范式预留出一条保守扩展路径：可选的 `teacher_silver` 样本增强。其设计原则是，允许把高置信的本地/API 评测结果转写成可训练的 silver JSONL，再作为额外训练输入并入 `train_fit` 主线；但这些 silver 样本只承担监督增强角色，不改变最终评测协议，也不把 API teacher 直接当作正式主表模型。
