...
class LANSScheduler:
    ...
    def __init__(
        ...
        signal_center: Optional[float] = None,
        signal_temperature: float = 0.25,
        ...
        if signal_center is None:
            signal_center = loss_baseline
        self.signal_center = float(signal_center)
        ...
        logger.info(
            ...
            f"EMA_decay={ema_decay}, SignalCenter={self.signal_center}, "
            f"SignalTemp={self.signal_temperature}, Loss_baseline={loss_baseline}, "
    ...
    def update_competence(self, batch_loss: float) -> float:
        ...
        with self._lock:
            ...
            # Step 1: 计算即时能力信号
            #   σ((center - L) / temp) ∈ (0, 1)
            #   center 控制“能力中点”，temp 控制斜率/灵敏度。
            instant_signal = self._sigmoid(
                (self.signal_center - batch_loss) / self.signal_temperature
            )
    ...
    def get_statistics(self) -> Dict:
        ...
        with self._lock:
            ...
            return {
                ...
                "signal_center": self.signal_center,
                "signal_temperature": self.signal_temperature,
