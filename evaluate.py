...
def main():
    ...
    eval_task_name = "eval_base" if args.base_only else "eval_local"
    ...
    # 3. 加载模型
    print("\n🔄 加载模型...")
    base_model_path = args.model_name_or_path or get_local_model_path(config['model'])
            ...
            if args.cot_eval_mode == "counterfactual" and bool(cf_cfg.get("enabled", True)):
                for _ in range(cf_num_perturb):
                    ...
                    if not perturbation.get("changed", False):
                        continue
                    cf_prompt = build_inference_prompt(
                        text=perturbed_text,
                        tokenizer=tokenizer,
                        use_oneshot=args.use_oneshot,
                        schema=getattr(adapter, "schema", None),
                    ...
                    cf_events, _ = json_parser.parse(cf_response)
                    if cf_events is None:
                        cf_events = []
                    if isinstance(cf_events, dict):
                        cf_events = [cf_events]
                    if not isinstance(cf_events, list):
                        cf_events = []
                    if args.pipeline_mode == "cat_lite":
